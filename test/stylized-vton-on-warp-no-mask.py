...
name = "stylized-warp-no-backgroundmask"
resize_or_crop = None
batchSize = "1"
gpu_ids = "0"
...
for style_f in ["la_muse.jpg"]: # only uses la_muse.jpg style
    ...
    for epoch in range(1,2): # 1 epoch
        for i, data in enumerate(dataset, start=epoch_iter):
            ...
            clothes_style = style_transfer_with_mask(style_img, warped_cloth.detach().cuda())
            normalized_clothes_style = normalizer(clothes_style.squeeze(0)).unsqueeze(0)
            normalized_clothes_style_masked = warped_edge*normalized_clothes_style
            ...
            gen_inputs = torch.cat([real_image.cuda(), normalized_clothes_style_masked, warped_edge], 1)
            gen_outputs = gen_model(gen_inputs)
            p_rendered, m_composite = torch.split(gen_outputs, [3, 1], 1)
            p_rendered = torch.tanh(p_rendered)
            m_composite = torch.sigmoid(m_composite)
            m_composite = m_composite * warped_edge
            p_tryon = normalized_clothes_style_masked * m_composite + p_rendered * (1 - m_composite)
            ...
            if step % 1 == 0:
                ...
                # e = normalized_clothes_style.cuda()
                # f = normalized_clothes_style_masked.cuda()
                # g = p_tryon
                # combine = torch.cat([a[0],b[0], flow_color, c[0], d[0], e[0], f[0],g[0]], 2).squeeze()
                # utils.save_image(
                #     combine,
                ...
                # # combine2 = torch.cat([a[0],b[0], c[0], d[0], e[0], f[0],g[0]], 2).squeeze()
