...
for style_f in ["la_muse.jpg"]: # only uses la_muse.jpg style
    ...
    for epoch in range(1,2): # 1 epoch
        for i, data in enumerate(dataset, start=epoch_iter):
            ...
            style_img_norm = normalizer(style_img.squeeze(0)).unsqueeze(0)

            clothes_style_mask = style_transfer_with_mask(style_img, clothes.detach().cuda(), content_mask=edge.cuda())
            clothes_style_mask_norm = normalizer(clothes_style_mask.squeeze(0)).unsqueeze(0)
            # p_try_on_style_norm = normalizer(p_try_on_style.squeeze(0)).unsqueeze(0)
            
            #import ipdb; ipdb.set_trace()
            
            flow_out = warp_model(real_image.cuda(), clothes_style_mask_norm.cuda())
            ...
            if step % 1 == 0:
                ...
                # c = style_img_norm.cuda()
                # d = clothes_style_mask_norm.cuda()
                # e = warped_cloth.cuda()
                # f = p_tryon

                # combine = torch.cat([a[0],b[0], c[0], d[0],flow_color, e[0], f[0]], 2).squeeze()
                ...
                # combine2 = torch.cat([a[0], b[0], c[0], d[0], e[0], f[0]], 2).squeeze()
                ...
                # combine3 = torch.cat([a[0], b[0], c[0]], 2).squeeze()
