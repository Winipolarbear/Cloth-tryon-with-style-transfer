...
for style_f in ["la_muse.jpg"]: # only uses la_muse.jpg style
    ...
    for epoch in range(1,2): # 1 epoch
        for i, data in enumerate(dataset, start=epoch_iter):
            ...
            p_try_on_style_everything_normalized = normalizer(style_transfer_with_mask(style_img, p_tryon.detach().cuda()).squeeze(0)).unsqueeze(0)
            p_try_on_style_masked = p_try_on_style_everything_normalized * warped_edge
            p_try_on_style = p_tryon*(1 - warped_edge) + p_try_on_style_masked
            # p_try_on_style_norm = normalizer(p_try_on_style.squeeze(0)).unsqueeze(0)
            ...
            if step % 1 == 0:
                ...
                # c = warped_cloth.cuda()
                # d = p_tryon.cuda()
                # e = normalizer(style_img.squeeze(0)).unsqueeze(0).cuda()
                # f = p_try_on_style_everything_normalized.cuda()
                # g = p_try_on_style_masked.cuda()
                # h = p_try_on_style.cuda()

                # combine = torch.cat([a[0],b[0], flow_color, c[0], d[0], e[0], f[0], g[0], h[0]], 2).squeeze()
                ...
                # combine2 = torch.cat([a[0], b[0], e[0], c[0],  g[0], h[0]], 2).squeeze()
                ...
                # combine3 = torch.cat([a[0], b[0], e[0]], 2).squeeze()
